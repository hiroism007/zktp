...
export class ZkEvent extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ZkEvent entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ZkEvent must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ZkEvent", id.toString(), this);
    }
  }

  static load(id: string): ZkEvent | null {
    return changetype<ZkEvent | null>(store.get("ZkEvent", id));
  }
